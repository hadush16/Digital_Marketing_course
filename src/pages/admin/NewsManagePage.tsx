...
export default function NewsManagePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await newsService.getAll({ search })
  ...
  const createMutation = useMutation({
    mutationFn: (newArticle: NewsFormData) => newsService.create(newArticle as any),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<NewsFormData> }) =>
      newsService.update(id, data as any),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-news'] })
      closeModal()
    },
    onError: (err: any) => {
      setFormError(err.response?.data?.message || 'Failed to update article')
    },
  })
  ...
  const openEditModal = (item: NewsArticle) => {
    ...
    setFormData({
      ...
      author: typeof item.author === 'string' ? item.author : item.author?.name || 'Ryoit Editorial',
    })
    setFormError(null)
    setIsModalOpen(true)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-extrabold text-2xl text-white">Manage Tech News</h1>
          <p className="text-sm text-dark-muted mt-1">
            Publish and manage technology, GSM news, and industry articles.
          </p>
        </div>
      ...
      <div className="glass-card-dark rounded-2xl border border-dark-border p-6 shadow-lg overflow-x-auto">
        {isLoading ? (
          <div className="py-12 text-center text-slate-400">Loading articles...</div>
        ) : isError ? (
          ...
          <div className="py-12 text-center text-slate-400">
            No articles found. Click "Add Article" to publish one.
          </div>
        ) : (
              ...
              {articles.map((item: NewsArticle) => (
                <tr key={item.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="py-4 px-4 text-xs text-slate-300">
                    {typeof item.author === 'string' ? item.author : item.author?.name || 'Ryoit Staff'}
                  </td>
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={editingArticle ? 'Edit Article' : 'Publish Tech News Article'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. New GSM Tools Released for 2025"
            />
          </div>

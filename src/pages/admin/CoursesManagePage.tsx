...
export default function CoursesManagePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await coursesService.getAll({ search })
  ...
  const createMutation = useMutation({
    mutationFn: (newCourse: CourseFormData) => coursesService.create(newCourse as any),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<CourseFormData> }) =>
      coursesService.update(id, data as any),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-courses'] })
      closeModal()
    },
  ...
  const openEditModal = (course: Course) => {
    ...
    setFormData({
      ...
      category: course.category || 'facebook-marketing',
      language: course.language || 'English',
      duration: typeof course.duration === 'number' ? course.duration : 60,
    })
    setFormError(null)
    setIsModalOpen(true)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-extrabold text-2xl text-white">Manage Courses</h1>
          <p className="text-sm text-dark-muted mt-1">
            Create, update, and manage digital marketing and technical repair courses.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Facebook Ads Mastery 2025"
            />
          </div>
